...
precipitation is only 2.6$\%$. Looking at Figure~\ref{crudemodel}, we see that the model precipitation total is lower than the actual precipitation
observed in the Summer of 2018. Furthermore, the precipitation seems to be
more frequent compared to the observed 2018 Summer data. The usage of 98th percentage for all parameters, which are intensity, precipitation event duration, and non-precipitation event duration, shows that the intensity 
for the model is generally good, except the model lacks the most intense values as expected from the using up to the 98th percentile.
...
\begin{figure}[t]
  ...
  \caption[Modified run using Summer 2018 climatology]
          {\label{crudermodel} One model run using the 100th percentile
            exponentials calculated from Summer 2018. The patterns with the
            modeled precipitation looks closer to actual
            precipitation. However, the details show they are different, the
            precipitation accuracy is only 2.6\% and the total precipitation
            is still underestimated. }
\end{figure}

\begin{figure}[b]
  \centering
  \includegraphics[width=0.65\textwidth]{Figures/whole2018_model.png}
...
\end{figure}
\clearpage
% We can try to apply our analysis on each of the models to see how the
% calculations for the exponentials differ between the models and the 
% observed data. 
% Need to quickly replace the figure as it flips the two labels, the bottom one is the observed and the top one is the predicted. 


\subsection{Discussion}\label{sec:spc_d}

Making predictions based on random guesses, however
well informed, has limited success. This is seen with the accuracy of
predicting precipitation. It shows that no matter how we do with the
model, such a model that characterizes the distribution of the three
parameters we analyzed in the precipitation data, can give you a similar
looking pattern of precipitation, but is not very accurate as seen with
the different graphs showing about a 3\% accuracy for the model. At the
same time, we have a dilema in which using the 98th percentile
exponentials will give you a preciptiation time series that looks nothing
like the observed model, but is close to the value of total precipitation
for the observed model. Using the 100th percentile exponentials, will give
you a preciptation time series that looks a lot like the observed
precipitation time series, but will have much less total precipitation
compared to the 98th percentile exponential and the observed total
precipitation. 

We can see that despite the flaws that we get from the exponential fits,
this gives us a good baseline to start off. As it tell us that there are other methods to look at in order to get a better model to predict precipitation. 
...
the precipitation data. It is clear that the observed data shows that
there are seasonal variations do matter as they have different patterns. 
Thus it validates the notion to split the analysis of the exponentials 
into the four seasons, as although its possible to have intense
precipitation to occur in the winter, it is not at all typical of what we
usually see in the winter and intense precipitation would usually occur in
the summer.   
...
% Why you need it. 
Although we have one model for the baseline for predicting precipitation
based on seasonally adjusted historical patterns of precipitation istelf, it
only makes point-based instantaneous predictions.  At 6--7\%, its prediction
...
As a result we turn to machine learning, which can
produce models based on experience and the usage of data.

%(Need to cite). 

For weather, forecasts are often based on numerical weather prediction
(NWP), which involve simulating the atmosphere and its conditions into the
...
equations such as physics conservation laws and ideal gas law. Some
complications of NWP include unable to simulate small-scale physics that can
affect outcomes, as well as the usage of ensembles, which try to give
...
and how intense such precipitation, which can be less computationally
intensive and can give us a good guide on what to expect if we encounter
such conditions. A method like Decision Tree can be used to select
variables, assess how relative importance of variables, handle missing
values, predict, and manipulate data. Decision Trees are made up of Nodes,
of which we have a root node, which represents a choice and a leaf node
which represents the final results of the decision tree. Branches are the
choices that are made from the root nodes \cite[]{DT}.
...
model. Linear regression will use regression to compute the best fit based
on the variables you give and will yield a linear function. Decision tree
divides the data up into different sections based on certain criteria. The
tree can be as small or as big as you want, but that will affect how the
model predict what you want.

% We also have neural networks, which inputs data and outputs data and can be used to simulate
...
\begin{figure}[t]
  ...
  \caption[ML linear regression run] {\label{ML_Linear}Training the model
    ...
    2019 data. Observations are the filled circles, predictions are the
    colored lines. As seen, the linear regression does not do a good job in
...
\begin{figure}[t]
	...
	\caption[ML Decision Tree regression run] {\label{ML_DT}Training the
          model using 2018 data and then using the decision tree model to
          predict 2019 data. Decision tree does a better job in matching the
          patterns that we see with the actual values. At the same time, we
          will need to see what training the model more would do to
          predictions.}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width = 0.75\textwidth]{Figures/predict_and_actual_2019_precip.png}
