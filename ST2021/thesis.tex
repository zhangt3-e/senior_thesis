...
to turn these numbers into a model based on climatology. We turned the data
we have into an exponential for the three parameters we looked at,
precipitation duration, non-precipitation duration, and intensity. The next
step is to use these exponentials to make a model to simulate precipitation
and compare such model results to the actual precipitation data.
...
model, we will look at how often it predicts precipitation and compare it to
whether the recorded data also has precipitation present. So basically if
predicted and recorded data shows precipitation at the same time, it counts
towards being accurate. This measure of accuracy is better than simply using
whether the model matches precipitation events with each other and
non-precipitation events with each other because non-precipitation events
accounts for 97\% of the duration, so the acccuracy is kind of skewed
towards the fact that most of the time series has no precipitation.
\clearpage 
...
\begin{figure}[t]
  ...
  \caption[First run using Summer 2018 climatology] {\label{crudemodel} One
    model run based using the 98 percentile exponentials calculated from
    Summer 2018.}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption[Modified run using Summer 2018 climatology]
          {\label{crudermodel}One model run using the 100th percentile
            ...
            modeled precipitation looks closer to actual precipitation when
            comparing side to side. Further details show they are different
            as shown with the precipitation accuracy as well as the total
            precipitation. }
\end{figure}
...
based on climatology, precipitation itself is not based on climatology, but
on other meteorological variables as well. As a result we turn to machine
learning, which can produce models based on expereince and the usage of
data.
%(Need to cite). 

For weather, forecasts are often based on numerical weather prediction(NWP),
which involve simulating the atmosphere and its conditions into the
...
There are a couple of methods from machine learning that can be used for
predicting precipitation. We have two methods, linear regression and
decision tree, that are used to predict precipitation based on the other
parameters that we give to the model. Linear regression will use regression
to compute the best fit based on the variables you give and will yield a
linear function. Decision tree divides the data up into different sections
based on certain criterias. The tree can be as small or as big as you want,
but that will affect how the model predict what you want.
