...
The better accuracy is also reflected in the pattern between the predicted LSTM and the observed data. In Figure~\ref{LSTM_minute_compare}, we have a pattern that more closely aligns to a diagonal, which would represent that both the predicted and observed data are of the same precipitation intensity. This is in start contrast to Figure~\ref{LSTM_hour}, which does not have a good overlap between predicted and observed values and looks closer to the Figure~\ref{DT_compare} of poor fits, particularly values that are not small like 0.01~mm or 0~mm. 
