...
With the exponential distributions calculated from the dataset, we are able to turn these numbers into a model based on climatology. We turned the data we have into an exponential for the three parameters we looked at, precipitation duration, non-precipitation duration, and intensity. The next step is to use these exponentials to make a model to simulate precipitation and compare such model results to the actual precipitation data. 
...
uniform distribution with those bins. In order to judge the accuracy of the model, we will look at how often it predicts precipitation and compare it to whether the recorded data also has precipitation present. So basically if predicted and recorded data shows precipitation at the same time, it counts towards being accurate. This measure of accuracy is better than simply using whether the model matches precipitation events with each other and non-precipitation events with each other because non-precipitation events accounts for 97\% of the duration, so the acccuracy is kind of skewed towards the fact that most of the time series has no precipitation. 
\clearpage 
...
\begin{figure}[t]
  ...
  \caption[First run using Summer 2018 climatology] {\label{crudemodel} One model run based using the 98 percentile exponentials calculated from Summer 2018.}
\end{figure}
...
\begin{figure}[t]
  ...
  {\label{crudermodel}One model run using the 100th percentile exponentials calculated from
    Summer 2018. The patterns with the modeled precipitation looks closer to actual precipitation when comparing side to side. Further details show they are different as shown with the precipitation accuracy as well as the total precipitation. }
\end{figure}
...
Why you need more work. Making predictions based on random guesses, however well informed, has limited success. This is seen with the accuracy of predicting precipitation
...
% Why you need it. 
Although we have one model for the baseline for predicting precipitation based on climatology, precipitation itself is not based on climatology, but on other meteorological variables as well. As a result we turn to machine learning, which can produce models based on expereince and the usage of data.
%(Need to cite). 

For weather, forecasts are often based on numerical weather prediction(NWP), which involve simulating the atmosphere and its conditions into the future. These models are incredibly complex as they need initial conditions (often from observations), boundary conditions, and the usage of governing equations such as physics conservation laws and ideal gas law. Some complications of NWP include unable to simulate small-scale physics that can affect outcomes, as well as the usage of ensembles, which try to give variety of predictions based on the small changes in initial conditions \cite[]{NWP}.  

Machine Learning can look at what conditions could yield us precipitation and how intense such precipitation, which can be less computationally intensive and can give us a good guide on what to expect if we encounter such conditions. A method like Decision Tree can be used to select variables, assess how relative importance of variables, handle missing values, predict, and manipulate data. Decision Trees are made up of Nodes, of which we have a root node, which represents a choice and a leaf node which represents the final results of the decision tree. Branches are the choices that are made from the root nodes \cite[]{DT}. 


\subsection{Method}

There are a couple of methods from machine learning that can be used for predicting precipitation. We have two methods, linear regression and decision tree, that are used to predict precipitation based on the other parameters that we give to the model. Linear regression will use regression to compute the best fit based on the variables you give and will yield a linear function. Decision tree divides the data up into different sections based on certain criterias. The tree can be as small or as big as you want, but that will affect how the model predict what you want. 

% We also have neural networks, which inputs data and outputs data and can be used to simulate (Need more info)
...
\begin{figure}
	...
	\caption[Comparing ML predicted values to actual precipitation values]{Comparing the predicted values from decision tree regression and the actual values of the 2019 precipitation data.  }
\end{figure}
